...
public class AttendService {
    ...
    public AttendDTO insert(AttendInsertDTO dto){
        ...
        if(dto.getBalance() < 0){
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "The balance must be greater than or equal to 0!");
        }
    ...
    public AttendDTO update(Long id, AttendUpdateDTO dto){
        ...
        try{
            ...
            if(entity.getBalance() < (dto.getBalance() * -1)){
                throw new ResponseStatusException(HttpStatus.NOT_FOUND, "This attend has only " + entity.getBalance() + " of balance!");
            }
